...
def create_generator(generator_inputs1, generator_inputs2, generator_outputs_channels):
    ...
    concat1 = tf.concat([generator_inputs1, generator_inputs2], 3) # 拼接两个tensor, 在维度3上（channel）128*128*5
    with tf.variable_scope("encoder_1_1"): # 分支1              
        output = conv(concat1, 3, 32, 1) # 3*3*32/1   128*128*32
    ...
    with tf.variable_scope("decoder_11"):
        ...
        layers.append(strided_convolved)
    concat3 = tf.concat([layers[-1], layers[-1-9]], 3) # 128*128*160
...
def main():
    ...
    # load model
    model = create_model(examples.blur_u, examples.pan, examples.mul)
